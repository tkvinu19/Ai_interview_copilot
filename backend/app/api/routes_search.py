...
def get_followup_questions(
    ...
    """
    Given an interview question, runs the full RAG pipeline,
    generates an answer, then returns 3 follow-up questions
    an interviewer would realistically ask next.
    """
    try:
        user_id = int(user_id)
        ...
        if not chunks:
            raise HTTPException(
                status_code=404,
                detail="No resume data found. Please upload your resume first."
            )
        ...
        # ── 5. Generate the answer first ─────────────────────────
        answer = generate_answer(query, relevant_chunks)

        # ── 6. Generate follow-up questions from answer ──────────
        followups = generate_followups(query, answer, relevant_chunks)
